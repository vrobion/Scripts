...
.NOTES
	...
	AUTHOR:	Valentin ROBION
	EMAIL:	vrobion@dynamips.com
	VERSION HISTORY:
        1.0   12/07/2023
#>
...
$UsersInfos = Get-ADUser -Filter { (Enabled -eq $True) -and (PasswordNeverExpires -eq $False)} -SearchBase "OU=Utilisateurs,OU=AS44,DC=as,DC=local" –Properties "DisplayName", "mail", "msDS-UserPasswordExpiryTimeComputed" | 
